...
- `.claude/skills/footer-map-button/SKILL.md` — **各ページ左下の「地図」ボタン・下部バーのサイズ統一手順書**
- `.claude/skills/design-quality/SKILL.md` — **見た目・動きの品質を上げるための実装ルール（新規UI中心）**
...
## 新規ページ下部への「地図」ボタン設置ルール

新しいページ（`about-*.html`等）の左下に「地図」ボタン＋白いバーを設置するときは、
**必ず `.claude/skills/footer-map-button/SKILL.md` の手順に従うこと。**
このスキルを見落として独自のサイズで実装すると、ページごとにボタンの大きさが
バラバラになる不具合が過去に起きている（2026-08-12〜13に発覚・修正）。
「地図」ボタンのサイズ（高さ34px・幅113px固定）とバーの高さ（51px）を
目分量で決めず、スキルに書かれた基準値をそのまま使い、実装後は必ず
ブラウザJSで実測して確認する。

## 見た目・動きの品質について

新しいUI（新規コンポーネント・ページ）を作るとき、または「見た目を良くして」
「動きを滑らかに」等の依頼を受けたときは、**`.claude/skills/design-quality/SKILL.md`
の手順に従うこと。** 単純な文言修正や1〜2行の微調整にはこのスキルを持ち出さない。

既存コードは数週間かけて1px単位でユーザーと確認しながら調整してきたものなので、
スキルのルール（余白・フォントサイズの刻み幅統一等）を既存箇所へ一括適用しない。
既存部分に手を入れる場合は、対象箇所を1つずつ明示してユーザーの確認を取ってから
進めること（2026-08-22、⚙メニュー・写真ライトボックスのフェード化、ミニマップの
リサイズ処理デバウンス化をこの手順で実施）。
...
- **プレビューは常にスマホ幅（375×812px、mobile preset）で確認する。**
  ...
  **2026-08-14追記：** プレビューを長時間操作しない間に表示幅がスマホサイズから
  ズレる事象が発覚したため、`mcp__Claude_Browser__navigate`（ページの再読み込み）
  呼び出し後にも同様のチェックを促すフックを追加した。navigateのたびに
  「現在の幅が375pxでなければresize_windowで直す（375pxで問題なければ何もしない・
  再度navigateしない）」という指示が注入される。※わざと「何もしなくてよい」と
  条件付きにしているのは、この指示への対応でnavigateを呼び直すと同じフックが
  再発火し無限ループになるため（実際に無限ループにならないことをテスト済み）。
