...
- バー高さ：51px
- ボタン高さ：34px
- ボタン幅：`index.html`の地図画面は3ボタンが同幅。その他のページの「地図」は113px
- バー下margin：18px
- ボタンがバー左端にある
...
- 幅のずれ：`index.html`の地図画面では3ボタンが同幅か確認する。その他のページでは`flex: 0 0 113px`またはgrid列113px固定を確認する。
...
修正後に同じコードで再計測する。375pxに加え、330pxまたは320px、必要に応じて390pxでも確認する。`index.html`の地図画面では3ボタンが各幅で均等か、その他のページでは地図ボタンが113pxから変化しないかを確かめる。
...
- 最新方針は、メイン地図画面だけ3ボタン均等、その他のページは地図ボタン113px固定。過去の「全ページ113px固定」を再適用しない。
...
- 375pxと狭幅で、メイン地図画面は3ボタンが均等幅、その他のページは地図ボタンが113px固定である。
- 共通CSSの全利用ページを確認した。
