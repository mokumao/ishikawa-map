...
1. Apps Scriptをウェブアプリとしてデプロイする。
   - 実行するユーザー：自分
   - アクセスできるユーザー：自分のみ
2. 発行された`/exec` URLを`admin/index.html`の`ADMIN_RESTORE_URL`へ設定する。
3. 管理者ページで非表示店舗の「履歴」→「再表示」を押す。
4. Googleの管理者アカウントで確認画面を開き、「地図に再表示する」を押す。

確認画面は10分間だけ有効な一度限りの確認情報を使います。GitHubトークンはApps Scriptのスクリプトプロパティだけに保存し、管理者ページへ埋め込みません。
