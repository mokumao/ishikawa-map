...
**チェーン店のsourceUrlは「個別店舗ページ」を優先する（2026-08-11新設）**
コンビニ・ガソリンスタンド等のチェーン店で、sourceUrlがチェーン本部の
汎用トップページ（例：`https://www.family.co.jp/`）になっていると、
情報源ボタンを押しても該当店舗の情報にたどり着けない（ユーザー指摘、
ファミリーマート石川白浜店で発覚）。チェーンの公式サイトに店舗検索機能
がある場合は、**必ずその店舗固有のページを探してsourceUrlに使う**。
- 探し方の例（ファミリーマート）：`store.family.co.jp` を開く→左上の
  「探す」→「店舗名」タブに切り替え→店舗名で検索→該当店舗をクリック→
  「詳しく見る」で個別ページ（`store.family.co.jp/points/XXXXX`形式）に
  遷移する。他チェーンも同様に公式サイト内の店舗検索を探す
